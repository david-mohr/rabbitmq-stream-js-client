...
class Client {
    ...
    async declarePublisher(params, filter) {
        ...
        const publisher = new publisher_1.StreamPublisher(streamPublisherParams, lastPublishingId, filter);
    ...
    async closeConsumer(extendedConsumerId) {
        ...
        const { streamInfos } = await this.connection.sendAndWait(new metadata_request_1.MetadataRequest({ streams: [activeConsumer.consumer.streamName] }));
    ...
    async getConnection(streamName, purpose, connectionClosedListener) {
        ...
        const cachedConnection = connection_pool_1.ConnectionPool.getUsableCachedConnection(purpose, streamName, this.connection.vhost, chosenNode.host);
        if (cachedConnection)
        ...
        connection_pool_1.ConnectionPool.cacheConnection(purpose, streamName, this.connection.vhost, newConnection.hostname, newConnection);
